...
export class GrammarService {
  ...
  /**
   * Analyze essay text for grammar, spelling, and style issues
   */
  ...
  /**
   * Fallback analysis when LanguageTool is not available
   */
  private fallbackAnalysis(text: string): GrammarAnalysis {
    ...
    // Simple heuristics for basic issues
   ...
   */
  private fallbackQuickCheck(text: string): {
    has_errors: boolean;
    error_count: number;
    score: number;
  } {
    const wordCount = text.split(/\s+/).length;
    const hasErrors = wordCount < 50; // Very basic heuristic
    
    return {
      has_errors: hasErrors,
      error_count: hasErrors ? 1 : 0,
      score: hasErrors ? 80 : 100
    };
  }
